...
10. **Mode "minor-safe"** : pour `age < 18` en style photo (`realistic`, `cinematic`), `buildCharacterImagePrompt` bascule sur `minorSafeLead` / `minorSafeModifiers` qui retirent les termes connus pour déclencher les classifiers anti-CSAM côté provider (`candid`, `raw photo`, `unedited`, `shot on iPhone`, `snapshot`, `skin texture`, `no makeup`, `no airbrushing`, `plastic skin`). On garde `Photographic portrait` + cues photo neutres → rendu photo plausible sans flag NSFW. La queue anti-glamour est aussi désactivée. Les styles illustrés (peinture, anime, cartoon, croquis) ne nécessitent pas de variante : pas de risque de flag.
11. **Détection NSFW serveur** : si fal renvoie `has_nsfw_concepts: true`, le handler retourne 422 + `code: NSFW_FILTERED`, `refundLastUsage` est appelé pour rembourser le crédit interne (fal facture quand même son côté). L'utilisateur reçoit un message explicite suggérant un style illustré.
