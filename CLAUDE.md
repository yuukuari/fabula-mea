...
│   │   ├── print-edition.ts     ← Constantes formats (trim sizes, papiers) + calculs (pages, dos, couverture dépliée, paginateContent)
...
│       ├── edition/             ← Module Édition : BookReader (3 modes : feuilleter/grille/taille-réelle), BookPreview3D, CoverSection, ConformityReport, LayoutRecapCard, LayoutChangeInfoDialog, PageRuler, PrintHelpModal (modales pédagogiques fonds perdus / marges / couverture à plat), wizard/ (StepTrimSize, StepInterior, StepCover, StepMetadata — réutilisés par EditionPrintPage)
