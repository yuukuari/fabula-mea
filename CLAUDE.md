...
| Correcteur ortho | nspell (Hunspell JS) + dictionary-fr | — |
| Tests | Vitest | 1.x |
...
│   │   ├── utils.ts             ← Helpers (generateId, now, CHAPTER_COLORS, countCharacters, countWordsFromHtml, countFromHtml, totalScenesCount, formatCount, countUnitLabel, isSpecialChapter, getChapterLabel, getChapterShortLabel, convertToSimpleDuration, computeEventEndDate, formatDuration...)
│   │   ├── calculations.ts      ← Calculs progression (mots/jour, scènes/jour, getTodayProgress, dailySnapshot, getPageEstimate simple + getSmartPageEstimate qui délègue à estimatePageCount si printEdition configurée)
...
│   │   ├── export-epub.ts       ← Génération EPUB 3 (métadonnées digitalEdition intégrées)
│   │   ├── export-pdf.ts        ← Export PDF via window.print() (mode `printReady` avec pages techniques)
│   │   ├── export-docx.ts       ← Export DOCX (Word) via bibliothèque docx
│   │   ├── export-cover-template.ts ← Gabarit couverture (SVG + guides fond perdu/sécurité/dos)
│   │   ├── export-cover-final.ts    ← Couverture finale print-ready (auto-composition simplifié OU flat+overlays avancé)
│   │   ├── cover-for-export.ts      ← Résout les couvertures pour EPUB/PDF/DOCX (crop du flat avancé + overlays via Canvas)
│   │   ├── export-printer-brief.ts  ← Dossier imprimeur (caractéristiques + instructions plateformes)
│   │   ├── print-edition.ts     ← Constantes formats (trim sizes, papiers — épaisseurs alignées standards KDP/IngramSpark) + calculs (pages, dos, couverture dépliée, paginateContent)
│   │   ├── cover-composition.ts ← Helpers composition couverture (modes Simplifié/Avancé, defaults, SpineRender, SPINE_MIN_TEXT_MM)
│   │   ├── conformity.ts        ← Moteur de checks de conformité (15 critères, 4 statuts : pass/warning/error/info, actions de navigation)
│   │   ├── migration.ts         ← Migrations (single-book → multi-book, clés localStorage, scènes → TimelineEvents)
│   │   ├── spellcheck-extension.ts ← Extension TipTap correcteur hybride (nspell + LanguageTool) + menu contextuel
│   │   ├── nspell-instance.ts   ← Singleton nspell (Hunspell FR), tokenizer, dictionnaire custom
...
│   │   ├── encyclopedia-helpers.ts ← CRUD partagé personnages/lieux/tags/worldNotes/maps + reorderCharacters/Places/WorldNotes (utilisé par useBookStore et useSagaStore)
...
│   │   ├── edition/             ← Module Édition (groupe sidebar dédié — voir point « Module Édition »)
│   │   │   ├── EditionOverviewPage.tsx    ← `/edition` — récapitulatif stepper + aperçu 3D
│   │   │   ├── EditionLayoutPage.tsx      ← `/edition/layout` — police, taille, interligne (commun)
│   │   │   ├── EditionCoversPage.tsx      ← `/edition/covers` — couvertures + gabarit PDF
│   │   │   ├── EditionPrintPage.tsx       ← `/edition/print` — wizard inline avec sections pliables (format, papier, couverture, métadonnées)
│   │   │   ├── EditionDigitalPage.tsx     ← `/edition/digital` — métadonnées EPUB (description, mots-clés, ISBN, licence, langue)
│   │   │   └── EditionExportPage.tsx      ← `/edition/export` — rapport de conformité + exports (EPUB, PDF, DOCX, PDF print-ready, dossier imprimeur)
...
│       ├── genealogy/           ← Arbre généalogique (GenealogyGraph, GenealogyNode, GenealogyEdge, GenealogyEditModal)
│       ├── edition/             ← Module Édition : BookReader (3 modes : feuilleter/grille/taille-réelle — feuilleter supporte marges alternées recto/verso, ombre du dos via CSS inset sur `.stf__item`, couverture avancée cropée via `ClippedFlatCover`), BookPreview3D (3D avec crop CSS du flat en mode avancé), CoverSection (toggle Simplifié/Avancé), CoverFlatPreview (aperçu SVG couv. dépliée, fontSize des overlays scalé avec widthPx/totalWidthMm × 0.2646), CoverAdvancedEditor (éditeur overlays drag/resize/rotation plein écran via Portal), ConformityReport, LayoutRecapCard, LayoutChangeInfoDialog, PageRuler, SpineWidthLabel/SpineWidthTooltip, PrintHelpModal (modales pédagogiques fonds perdus / marges / couverture à plat), wizard/ (StepTrimSize, StepInterior, StepCover, StepMetadata — réutilisés par EditionPrintPage)
...
Un livre contient :
- **Characters** — Personnages avec relations mutuelles, évolution, événements clés, avatar rond avec `imageOffsetY` (pourcentage 0-100 pour le centrage vertical de l'image), `order?` (position dans la liste, définie par drag-and-drop), `genealogy?` (arbre généalogique : parents, conjoints, enfants avec rôles typés et rôles custom)
- **Places** — Lieux typés (ville, bâtiment, paysage...) avec connexions, `order?` (position dans la liste, définie par drag-and-drop)
...
- **WorldNotes** — Notes de worldbuilding catégorisées, `order?` (position dans la liste, définie par drag-and-drop)
- **NoteIdeas** — Notes et idées libres avec éditeur TipTap (toolbar complète : titres, formatage, alignement, listes, checklists, citations, images, liens), affichées en grille de cartes avec aperçu HTML du contenu (titre facultatif, contenu obligatoire), vue détail au clic, `order` (position dans la liste, définie par drag-and-drop)
...
- **Layout** — Paramètres de mise en page du livre (`BookLayout`) : police (`BookFont`), taille (`BookFontSize` : 10-18pt), interligne (`BookLineHeight` : 1.0-2.0), images de couverture (`coverFront`, `coverBack`, `coverSpine` en base64 ou URL CDN), et deux sous-objets optionnels : `printEdition?: PrintEdition` (paramètres impression papier) et `digitalEdition?: DigitalEdition` (métadonnées EPUB). **Le layout est toujours au niveau du livre** (`BookProject.layout`) ; il n'est plus partagé au niveau saga depuis la refonte du module Édition. Par défaut : Times New Roman 12pt, interligne 1.5. Ces paramètres s'appliquent à l'éditeur TipTap, au mode relecture, et aux exports EPUB/PDF/DOCX. Polices disponibles : Times New Roman, Georgia, Garamond, Crimson Text, Lora, Merriweather, EB Garamond, Libre Baskerville (chargées via Google Fonts). L'éditeur permet de changer la police (`FontFamily`) ET la taille (`FontSize`, via `src/lib/font-size-extension.ts`) d'un texte sélectionné. Les titres h1/h2/h3 ont été retirés de la barre d'outils de l'éditeur de scènes. Un `onSelectionUpdate` force le re-rendu des sélecteurs police/taille pour refléter la sélection courante.
- **PrintEdition** — Sous-objet de `BookLayout` pour la préparation à l'impression papier. Champs : `trimSize` (`TrimSizeId` : poche/a5/6x9/royal/digest), `paperType` (`PaperType` : white_80/cream_80/white_90), `margins` (`PrintMargins` : topMm, bottomMm, innerMm, outerMm), `bleedMm`, `isbn?`, `publisher?`, `printDate?` (YYYY-MM). Modifié via `updatePrintEdition(data: Partial<PrintEdition>)` dans `useBookStore`. Dimensions des formats, épaisseurs papier et calculs (nombre de pages estimé, largeur du dos, couverture à plat) sont dans `src/lib/print-edition.ts`.
- **DigitalEdition** — Sous-objet de `BookLayout` pour les métadonnées EPUB. Champs : `description?` (texte long pour les boutiques), `keywords?: string[]` (mots-clés `dc:subject`), `isbnDigital?`, `rights?` (`BookRights` : all_rights_reserved / cc_by / cc_by_sa / cc_by_nc / cc_by_nc_sa / cc_by_nd / cc_by_nc_nd / public_domain), `language?` (ISO 639-1), `publisher?`. Modifié via `updateDigitalEdition(data: Partial<DigitalEdition>)` dans `useBookStore`. L'export EPUB (`src/lib/export-epub.ts`) injecte ces valeurs dans les balises OPF `dc:*`.
- **CoverMode** (`'simplified' | 'advanced'`, default `'simplified'`) — détermine comment la couverture dépliée est composée à l'export.
- **CoverSimplifiedConfig** — Sous-objet de `BookLayout` (mode simplifié). Champs : `spineColor?` (hex, default bordeaux), `spineShowTitle?` (default `true` si dos ≥ 6 mm), `spineFontFamily?` (default = police du livre), `spineTextColor?`, `spineOrientation?` (`'ttb'` tête en haut / `'btt'` tête en bas). L'utilisateur upload juste 1ère + 4ème ; le dos est **auto-composé** (couleur + titre/auteur vertical). Modifié via `updateCoverSimplified`.
- **CoverAdvancedConfig** — Sous-objet de `BookLayout` (mode avancé). Champs : `flatImage?` (URL CDN ou base64 de la couverture dépliée entière), `overlays?: CoverTextOverlay[]`. L'utilisateur upload une image de fond puis ajoute des textes positionnés. Modifié via `updateCoverAdvanced`.
- **CoverTextOverlay** — Élément texte positionné sur la couverture avancée. Coordonnées en pourcentages (`xPct`, `yPct`, `widthPct`, `heightPct` ∈ 0-100) pour scaler entre preview/export. Attributs : `rotation` (°), `content`, `fontFamily`, `fontSize` (px), `color`, `fontWeight`, `fontStyle`, `textAlign`.
- **CustomDictionary** — Liste de mots personnalisés par livre (`customDictionary?: string[]`) pour le correcteur orthographique. Mots ajoutés via « Ajouter au dictionnaire » dans l'éditeur. Automatiquement enrichi avec les noms de personnages, lieux et notes univers.

### Sagas

Les sagas permettent de regrouper plusieurs livres partageant une encyclopédie commune :
- **SagaMeta** — Métadonnées : id, titre, description, auteur, genre, nombre de livres, dates, writingMode, countUnit. **N'a pas de `layout`** : la mise en page (police, taille, interligne) et les couvertures sont strictement au niveau du livre (`BookProject.layout`), même pour les livres appartenant à une saga.
...
15. **Sidebar** : la navigation livre commence par un lien top-level « Vue d'ensemble » (tableau de bord), puis des groupes dépliables — Encyclopédie (Personnages, Lieux, Cartes, Univers), Manuscrit (Chronologie, Chapitres, Relectures), Suivi (Progression, Objectifs), **Édition** (Récapitulatif, Mise en page, Couvertures, Édition papier, Édition numérique, Export), Notes & Idées, Paramètres, et Aide & Support (Créer un ticket avec style bouton, Tickets). L'expansion automatique suit la route active. La sélection d'un livre redirige vers `/encyclopedia` (tableau de bord). La Chronologie est en première position du groupe Manuscrit car c'est souvent la première étape de conception d'un livre. **Édition** forme son propre groupe top-level (après Suivi) et n'est plus dans Manuscrit. Les `NavLink` supportent un flag `end?: boolean` pour que `/edition` (Récapitulatif) ne reste pas actif sur les sous-routes `/edition/layout`, `/edition/covers`, etc.
...
23. **Couvertures et page de titre** : la 1ère de couverture et la page de titre (titre + auteur) apparaissent dans le manuscrit (ChaptersPage), les relectures (nav « Page de titre » + contenu), le PDF (avant la page de titre) et l'EPUB (métadonnées OPF `properties="cover-image"`). La 4ème de couverture apparaît à la fin dans le manuscrit, les relectures (nav « 4ème de couverture ») et le PDF. Dans l'EPUB, seule la 1ère de couverture est intégrée aux métadonnées. La **tranche (`coverSpine`)** est également supportée : upload dans `/edition/covers`, rendu en 3D dans `BookPreview3D` quand l'image est fournie (sinon fallback bordeaux avec titre vertical). Les couvertures acceptent base64 (dev) ou URL CDN (prod Vercel Blob).
...
29. **Module Édition** : groupe top-level dans la sidebar (après Suivi) avec 6 sous-pages dédiées dans `src/pages/edition/`. L'architecture sépare clairement ce qui est **commun** aux formats (mise en page, couvertures), **spécifique au papier**, et **spécifique au numérique**.
    - **`/edition` — EditionOverviewPage** : dashboard avec un « stepper » de 5 étapes cliquables (Mise en page, Couvertures, Édition papier, Édition numérique, Export) chacune avec statut ✓/○ et résumé dynamique, + aperçu 3D du livre. Le `BookReader` est monté conditionnellement (`{readerOpen && <BookReader…/>}`) pour éviter la pagination à chaque re-render.
    - **`/edition/layout` — EditionLayoutPage** (commun) : police, taille, interligne + aperçu live. Auto-save. Déclenche une modale `LayoutChangeInfoDialog` qui explique que seul le texte « par défaut » est affecté. Cette section a été **déplacée depuis SettingsPage** vers ce groupe.
    - **`/edition/covers` — EditionCoversPage** (commun) : upload 1ère/4ème/dos via `CoverSection`, avec schéma SVG dimensionné si `printEdition` est configurée. Bouton « Télécharger le gabarit » qui appelle `exportCoverTemplate` (SVG aux dimensions exactes de la couverture dépliée avec guides : ligne rouge = coupe, pointillés verts = zone sécurité 5mm, pointillés bleus = dos, repères de coupe aux coins).
    - **`/edition/print` — EditionPrintPage** : 4 sections pliables numérotées (Format / Intérieur / Couverture / Métadonnées) avec **auto-save** sur chaque modification (pas de draft local, pas de bouton Enregistrer). Une seule section ouverte à la fois. Réutilise les composants `StepTrimSize`, `StepInterior`, `StepCover`, `StepMetadata` de `src/components/edition/wizard/`. Liens d'aide contextuels (`HelpTrigger`) vers les modales `PrintHelpModal` sur les thèmes `safety` (marges de sécurité), `bleed` (fonds perdus) et `flat-cover` (couverture à plat). Carte « Récapitulatif » en bas avec 4 métriques live (Format, Pages estimées, Largeur du dos, Couverture à plat).
    - **`/edition/digital` — EditionDigitalPage** : description longue (textarea + compteur), mots-clés (tag input avec Entrée/virgule/paste/Backspace), ISBN numérique avec validation 10/13 chiffres, langue (ISO 639-1 sélectionnable), éditeur numérique, licence (8 radio cards : Tous droits réservés + 6 Creative Commons + Domaine public). Auto-save.
    - **`/edition/export` — EditionExportPage** : affiche le **Rapport de conformité** (voir point 42) en haut. Les exports sont regroupés visuellement en deux sections avec headers : (1) **« Formats numériques & partage »** — EPUB / PDF de lecture / DOCX (disabled si aucun chapitre) ; (2) **« Impression papier »** (visible seulement si `printEdition` configuré) — PDF prêt à imprimer avec pages techniques auto-générées (blanche après la couverture, copyright avec ISBN/éditeur/date, blanche avant la 4ème) via `exportPdf(book, { printReady: true })`, et « Dossier imprimeur » via `exportPrinterBrief` qui génère un HTML imprimable A4 avec caractéristiques complètes, checklist de fichiers, et instructions spécifiques par plateforme (Coollibri, KDP, IngramSpark, Lulu).
    - **Helper partagé** : `totalScenesCount(scenes, unit)` dans `src/lib/utils.ts` — factorise le calcul de total mots/signes utilisé dans les 6 pages Édition et le moteur de conformité.
    - **Typographie dans l'EPUB** : `exportEpub` utilise en priorité les métadonnées `digitalEdition` (`isbnDigital`, `language`, `publisher`, `description`, `keywords`, `rights` → `dc:*`), avec fallback sur les champs historiques (synopsis, genre, printEdition.isbn/publisher).
    - **BookReader 3 modes** : le reader plein écran (`src/components/edition/BookReader.tsx`) propose un toggle en header entre `flip` (livre page-flip, existant), `grid` (thumbnails cliquables → bascule en taille réelle) et `actual-size` (une page à la taille physique réelle à 96 CSS DPI avec règle cm/mm via `PageRuler`). Le `useEffect` d'init page-flip ne se déclenche qu'en mode `flip` et dépend de `displayPages` (réinitialise sur changement de contenu). Le handler `on('flip')` est guardé contre les événements post-`destroy`.

42. **Rapport de conformité (`src/lib/conformity.ts` + `ConformityReport.tsx`)** : moteur de vérifications automatiques avant export. Types : `ConformityStatus = 'pass' | 'warning' | 'error' | 'info'`, `ConformityCategory = 'common' | 'print' | 'digital'`, `ConformityCheck { id, category, title, status, message, solution?, action? }`. Fonction `checkConformity(input)` retourne ~15 checks (titre, auteur, contenu, scènes incomplètes, mise en page, configuration papier, marge intérieure ≥ 15mm, fond perdu ≥ 3mm, ISBN papier valide, nombre de pages multiple de 4, couvertures, ISBN numérique, métadonnées numériques…). Fonction `summarizeConformity(checks)` retourne compteurs + flag `readyToExport` (aucune erreur). UI : barre de résumé colorée (compteurs par statut + badge global « Prêt pour l'export » ou « Erreurs à corriger »), cartes groupées par catégorie, boutons d'action qui redirigent vers la page de correction appropriée. Intégré dans `EditionExportPage`.

43. **Modales pédagogiques Édition** : `PrintHelpModal` (`src/components/edition/PrintHelpModal.tsx`) propose 3 topics (`bleed`, `safety`, `flat-cover`) avec illustrations SVG inline. Affichable via le composant `<HelpTrigger topic="..." label="..." onClick={setHelpTopic} />` — un petit bouton ⓘ + label qui ouvre la modale. Utilisée dans `EditionPrintPage` en haut des sections Intérieur (marges de sécurité) et Couverture (fonds perdus + couverture à plat). Les illustrations montrent 3 exemples ✗/✓ pour les fonds perdus, un schéma de double page pour les marges, et une vue dépliée 4ème+dos+1ère pour la couverture à plat.

44. **Estimation unifiée des pages** : `getSmartPageEstimate(count, unit, layout, chapterCount)` dans `src/lib/calculations.ts` est le point d'entrée unique pour estimer un nombre de pages. Quand `layout.printEdition` est configurée, délègue à `estimatePageCount` (`src/lib/print-edition.ts`) qui tient compte du format, papier, marges, police et nombre de chapitres. Sinon fallback sur `getPageEstimate` (~250 mots/page). Utilisé dans `ProgressionPage` (carte principale + Longueur du livre + modal de configuration), `EncyclopediaPage` (carte Édition), `EditionPrintPage` (récapitulatif), `BookPreview3D`, `EditionCoversPage` (pour le gabarit), `conformity.ts` (check nombre de pages). **Le filtrage des chapitres « vrais »** (non front/back matter) utilise `isSpecialChapter` de façon cohérente dans toutes les pages. Le composant `PageInfoTip` (ProgressionPage) affiche un texte adapté : « Estimation basée sur votre édition papier » si `hasPrintEdition`, sinon « Estimation format livre de poche · configurable dans Édition ».

45. **Vue d'ensemble — carte Édition** : `EncyclopediaPage` (route `/encyclopedia`, tableau de bord du livre) contient une carte « Édition » pleine largeur (`lg:col-span-2`) affichant : format configuré + pages estimées (via `getSmartPageEstimate`), barre de progression sur 4 critères (`printEdition`, `coverFront`/`coverBack`, `digitalEdition`, `chapters.length > 0`), et un bouton « Feuilleter le livre » qui ouvre directement le `BookReader` en overlay. Le reader est monté conditionnellement (`{readerOpen && <BookReader/>}`) pour ne pas lancer la pagination à chaque render du dashboard.

46. **Couvertures — 2 modes** : la page `/edition/covers` (via `CoverSection`) propose un **toggle Simplifié/Avancé** :
    - **Simplifié (par défaut)** — L'utilisateur téléverse uniquement la **1ère** et la **4ème de couverture**. Le **dos est auto-composé** à partir de `CoverSimplifiedConfig` : couleur unie + (optionnellement) titre + auteur rendus verticalement. Paramètres : couleur du dos (color picker), checkbox « Afficher titre/auteur » (désactivée si dos < `SPINE_MIN_TEXT_MM = 6 mm`), police, couleur du texte, orientation (ttb standard KDP ou btt européen). Un **aperçu SVG couverture dépliée** (`CoverFlatPreview`) montre le résultat en temps réel.
    - **Avancé** — L'utilisateur téléverse **une seule image** : la couverture dépliée complète (back + dos + front + fond perdu) préparée dans Photoshop/Canva/GIMP avec le gabarit. L'app fournit un **éditeur d'overlays texte** (`CoverAdvancedEditor`) permettant d'ajouter des éléments de texte positionnables par-dessus : drag pour déplacer (pointer events + `setPointerCapture`), handle rose pour redimensionner, handle bleu circulaire pour la rotation, panneau de propriétés (police, taille, couleur, gras/italique, alignement, rotation, contenu). Touche `Suppr`/`Backspace` pour supprimer l'overlay sélectionné, `Escape` pour désélectionner. Coordonnées stockées en pourcentages → scalables à toutes les résolutions. Implémentation vanilla (divs absolute + SVG guides), aucune lib externe.
    - **Exports** : `/edition/covers` offre 2 boutons quand `printEdition` configuré — « **Couverture finale** » via `exportCoverFinal()` (génère le PDF print-ready complet avec composition automatique selon le mode) et « **Gabarit vierge** » via `exportCoverTemplate()` (PDF avec uniquement les guides, pour ceux qui composent hors de l'app).
    - **Dos auto dans BookPreview3D** : en mode simplifié, le livre 3D rend le dos avec la couleur et le texte vertical choisis (fallback bordeaux + titre/auteur avec police de base). Le `resolveSpineRender(layout, title, author, spineWidthMm)` (`src/lib/cover-composition.ts`) est la source de vérité pour le rendu, utilisé partout (CoverFlatPreview, BookPreview3D, exportCoverFinal).
    - **`coverSpine` legacy** : le champ reste dans `BookLayout` pour rétrocompat (anciens livres qui avaient uploadé un dos image) mais n'est plus exposé dans la nouvelle UI simplifiée. Si présent, il prend le dessus sur l'auto-composition.

47. **Fiabilité du dos & standards d'impression** : la largeur du dos est calculée via `calculateSpineWidth(pageCount, paperType) = (pageCount / 2) × thicknessMm`. Les épaisseurs `PAPER_TYPES` sont **alignées sur les standards KDP Amazon / IngramSpark** : blanc 80g = 0.1144 mm/feuille (0.0572 mm/page), crème 80g = 0.127 mm/feuille (0.0635/page), blanc 90g = 0.130 mm/feuille. C'est une **estimation à ±10 %** — la vraie épaisseur dépend du fournisseur, de la colle et du pelliculage. Un composant `<SpineWidthTooltip />` (`src/components/edition/SpineWidthLabel.tsx`) affiche une icône info avec tooltip « Estimation ±10 % — l'imprimeur calcule le dos définitif » ; il est utilisé partout où la valeur apparaît (récap Édition papier, CoverSection, conformity report, dossier imprimeur). Sous 6 mm, un badge « Dos trop fin pour du texte » apparaît et désactive l'option titre vertical.
30. **Ordre par drag-and-drop** : les personnages, lieux, fiches univers et notes & idées ont un champ `order?` (ou `order` pour NoteIdea) qui définit leur position dans la liste. Les pages correspondantes (`CharactersPage`, `PlacesPage`, `WorldPage`, `NotesIdeasPage`) permettent de les réordonner par drag-and-drop (`@dnd-kit`, `rectSortingStrategy`). L'ordre des personnages est aussi utilisé dans la chronologie (`TimelinePage`) pour afficher les rangées dans l'ordre choisi. Le drag-and-drop est désactivé quand une recherche ou un filtre est actif. Les fonctions `reorderCharacters`, `reorderPlaces`, `reorderWorldNotes` sont disponibles dans `useBookStore`, `useSagaStore` et `useEncyclopediaStore`. `reorderNoteIdeas` est uniquement dans `useBookStore` (les notes ne font pas partie de l'encyclopédie partagée des sagas). Les helpers partagés sont dans `encyclopedia-helpers.ts`.
30b. **Filtre personnages livre/généalogie** : la page Personnages affiche des filtres en badges (« Tous / Livre / Généalogie ») au-dessus de la barre de recherche, visibles uniquement s'il existe des personnages de généalogie (`hideFromRelationshipGraph: true`). Tous les personnages apparaissent dans la même grille. Les personnages de généalogie affichent un badge « Généalogie » (icône `GitFork`) dans leur `CharacterCard`. Le drag-and-drop est désactivé quand un filtre autre que « Tous » ou une recherche est active.
...
32. **Graphe des relations** : les edges du graphe (`RelationshipGraph`) normalisent `source`/`target` par tri d'ID pour garantir que les courbes entre une même paire de personnages aillent toujours dans la même direction perpendiculaire, quel que soit l'ordre d'itération des personnages. Le champ `arrowTarget` stocke la vraie cible des relations non réciproques pour dessiner la flèche dans le bon sens. **Réciprocité** : les types `friend`, `enemy`, `colleague`, `family` (`ALWAYS_RECIPROCAL_TYPES`) sont toujours réciproques (relation créée/mise à jour/supprimée des deux côtés automatiquement). Les types `lover`, `mentor`, `rival`, `custom` sont optionnellement réciproques via une checkbox « Relation réciproque » visible à la création ET en édition. La suppression d'une relation supprime aussi la relation inverse si elle existe (`CharacterDetail.handleDeleteRelationship`). Le tooltip du graphe affiche « (non réciproque) » pour les relations à sens unique.
...
34. **Synchronisation cloud robuste** : la sauvegarde cloud utilise `cloudSaveWithRetry` (3 tentatives, backoff exponentiel). Le `loadBook` initialise un état minimal (`id: bookId`) même sans cache localStorage, pour que les données du serveur puissent s'appliquer. Des guards (`bookIdAtSave`, `loadedAt`) empêchent les promesses stale de corrompre l'état. Le debounce est annulé (`cancelPendingSave()`) lors du changement de livre. **Guard `getTodayProgress`** : la fonction refuse de créer un snapshot localStorage quand `currentTotal === 0` (données pas encore chargées depuis le cloud), et répare les snapshots empoisonnés (total=0 avec données réelles). Sans ce guard, la race condition `loadBook(scenes=[])` → `getTodayProgress(0)` → snapshot `{total:0}` faisait apparaître tout le contenu du livre comme "écrit aujourd'hui".
35. **Tests unitaires** : Vitest en environnement `node` avec setup file (`src/__tests__/setup.ts` pour polyfill `crypto` en Node 18). Tests dans `src/__tests__/` (8 fichiers, ~209 tests). Couvrent : retry avec backoff (`cloudSaveWithRetry`), historique de versions (extractStats, dedup, restore, saga data), optimisation daily snapshot, calculs de progression et objectifs (`calculations.ts` : incluant `getTodayProgress` avec guard race condition cloud), fonctions utilitaires (`utils.ts` : durées, comptage mots/signes, labels chapitres, formatage), CRUD encyclopédie (`encyclopedia-helpers.ts` : personnages, lieux, tags, worldNotes, maps, pins, relations, keyEvents, workflow réciproque), sanitization export XHTML (`export-shared.ts` : escapeXml, cleanHtml), helpers upload (`upload.ts` : isBase64, isUrl). Voir `src/__tests__/CLAUDE.md` pour les détails.
...
39. **Arbre généalogique** : chaque personnage peut avoir une `genealogy` (`CharacterGenealogy`) avec parents, conjoints et enfants. Types de rôles : `GenealogyParentRole` (père, mère, beau-père, belle-mère, autre), `GenealogySpouseRole` (mari, femme, compagnon, compagne, autre), `GenealogyChildRole` (fils, fille, beau-fils, belle-fille, autre). Rôle « autre » permet un `customRole` libre. Conjoints : ordonnables (drag), toggle « Ensemble/Séparés » (`current`). Enfants groupés par conjoint. **GenealogyEditModal** : modale d'édition avec fiche personnage collapsible (prénom, nom, surnom, sexe, description, dates naissance/décès) et sections relations (parents, conjoints, enfants). **AddLinkDialog** : ajout de relation via personnage existant ou création d'un nouveau. L'exclusion est globale : un personnage déjà lié dans n'importe quel rôle (parent, conjoint, enfant) ne peut pas être ajouté à nouveau. Les nouveaux personnages sont créés avec `hideFromRelationshipGraph: true`. **Fichiers** : `src/components/genealogy/` (GenealogyGraph, GenealogyNode, GenealogyEdge, GenealogyEditModal). Store : `addGenealogyParent`, `removeGenealogyParent`, `addGenealogySpouse`, `removeGenealogySpouse`, `updateGenealogySpouse`, `addGenealogyChild`, `removeGenealogyChild`, `reorderGenealogySpouses` dans `useEncyclopediaStore`.
48. **BookReader — marges alternées, ombre du dos, couverture avancée** : le reader (`src/components/edition/BookReader.tsx`) applique plusieurs règles fidèles au rendu d'un livre physique :
    - **Marges alternées** : la marge intérieure (côté reliure) est à gauche pour les pages recto (numéro impair) et à droite pour les pages verso (numéro pair). Contrôlé par `isVerso = pageNumber > 0 && pageNumber % 2 === 0`. Les couvertures et pages blanches (`pageNumber = 0`) restent en disposition recto.
    - **Ombre du dos** : appliquée en CSS `box-shadow: inset` sur chaque `.stf__item > div` dans `src/index.css` (sélecteurs `.stf__item.--right` et `.stf__item:not(.--right)`). L'ombre fait partie de chaque page, donc elle suit naturellement la page lors du feuilletage (pas de z-index à gérer).
    - **Couverture avancée** : en mode `advanced` avec `flatImage` (couverture dépliée), le composant `ClippedFlatCover` réutilise `CoverFlatPreview` dans un wrapper `overflow: hidden` qui clippe aux dimensions du trim front/back. Les overlays de texte sont rendus sur le preview et visibles si elles tombent dans la zone clippée. Les dimensions du flat sont calculées à la volée via `calculateCoverDimensions` + `estimatePageCount`.
    - **Page-flip et StrictMode** : l'init de `page-flip` est différée par `setTimeout(…, 0)` avec flag `aborted` pour que le double-mount de `React.StrictMode` en dev n'altère pas le DOM (la 1ère init est annulée avant de toucher au DOM).
    - **Overflow** : `overflow-hidden` en mode Feuilleter (clippe l'animation de courbure), `overflow-auto` + `items-start` en Grille et Taille réelle pour que la scrollbar parte du haut.
    - **Capacité de pagination** : `charsPerPage` (dans `src/lib/print-edition.ts`) applique un facteur de sécurité de 0.88 pour éviter que la dernière ligne soit clippée.
    - **Estimation typographique cohérente** : les scales de police en modes Feuilleter / Grille / Taille réelle sont dérivés du ratio `pageRendu / pageTailleRéelle` pour que le texte ait la même proportion partout.

49. **Couvertures dans les exports EPUB/PDF/DOCX** : le helper `resolveCoverForExport` (`src/lib/cover-for-export.ts`) est appelé dans `EditionExportPage` avant chaque export et alimente `ExportBook.resolvedCoverFront/Back`. Comportement :
    - Mode **simplifié** : retourne directement `layout.coverFront` / `coverBack`.
    - Mode **avancé** : crop la `flatImage` au trim front ou back via Canvas API (pas de librairie externe), puis dessine les overlays de texte visibles dans la zone via `ctx.fillText` + `ctx.rotate`. Retourne un data URL JPEG.
    - **CORS en prod** : les URLs Vercel Blob servent `Access-Control-Allow-Origin: *`, donc `img.crossOrigin = 'anonymous'` charge l'image sans tainter le canvas. En dev, les covers sont des data URLs base64 — pas de souci CORS.
    - **Fallback** : si le chargement de l'image échoue (CORS inattendu, décodage), le helper retourne l'URL du flat entier pour que l'export ait au moins une image.
    - Les 3 exports (`export-pdf.ts`, `export-epub.ts`, `export-docx.ts`) préfèrent `book.resolvedCoverFront/Back` à `layout.coverFront/Back` quand fourni.

50. **BookPreview3D — couverture avancée** : le preview 3D (`src/components/edition/BookPreview3D.tsx`) supporte le mode avancé via CSS `backgroundImage` + `backgroundSize/backgroundPosition` calculés en pourcentages pour clipper le flat image à la portion 1ère (face avant), 4ème (face arrière) ou dos (tranche). En mode simplifié, fallback aux champs `coverFront`/`coverBack`/`coverSpine` et au rendu auto-composé du dos.

51. **Marges alternées dans les exports print** :
    - **PDF** : règles CSS `@page :right` et `@page :left` dans `export-pdf.ts` — Chromium (moteur de `window.print()`) alterne automatiquement sur les pages verso/recto.
    - **DOCX** : la librairie `docx` npm n'expose pas `mirrorMargins`. On post-traite le .docx généré via JSZip (`injectMirrorMargins` dans `export-docx.ts`) pour injecter `<w:mirrorMargins/>` dans `word/settings.xml`. La config `{ left: innerMm, right: outerMm }` est alors interprétée correctement par Word.
    - **EPUB** : format reflowable — pas de marges fixes ni d'alternance (les lecteurs gèrent eux-mêmes).

52. **fontSize des overlays de couverture** : convention unique partagée entre éditeur, preview et exports — `fontSize` est stocké en « CSS px à l'échelle 1:1 96 DPI » (donc 1 px ≈ 0.2646 mm). Chaque renderer scale à son canvas : `canvas_font_px = stored_fontSize × (canvasWidthPx / totalWidthMm) × 0.2646`. Dans `CoverFlatPreview`, l'éditeur fullscreen (`CoverAdvancedEditor`) et le helper `cover-for-export.ts`. Dans `export-cover-final.ts` : les SVG utilisent **des valeurs unitless** (car le viewBox est en mm) — ne JAMAIS utiliser le suffixe `mm` en CSS dans un foreignObject à viewBox en mm, il provoque une inflation × 3.78 (cause de l'ancien bug de texte géant).

40. **Correcteur orthographique hybride + menu contextuel** : le correcteur utilise deux couches complémentaires. (1) **Orthographe** via `nspell` (port JS de Hunspell) avec dictionnaire français chargé depuis `/dictionaries/fr.aff` et `/dictionaries/fr.dic` — vérification locale, instantanée, hors-ligne, soulignement rouge ondulé. (2) **Grammaire** via LanguageTool API — envoi uniquement du paragraphe modifié (détection par hash), soulignement bleu/violet ondulé. Les catégories TYPOS/TYPOGRAPHY/STYLE/CASING et les règles `FR_SPELLING_RULE`/`HUNSPELL_RULE`/`MORFOLOGIK_RULE_FR` sont désactivées côté LanguageTool (nspell gère l'orthographe). **Dictionnaire personnalisé** : `BookProject.customDictionary?: string[]` — mots ajoutés par l'utilisateur via le bouton « Ajouter au dictionnaire » dans la popup de suggestions. **Pré-remplissage** : les noms de personnages (name, surname, nickname), lieux (name) et notes univers (title) sont automatiquement ajoutés au dictionnaire via `getCustomWords()` (ref dynamique, pas de recréation de l'éditeur). **Menu contextuel** (clic droit, remplace le menu natif du navigateur) : architecture multi-niveaux avec navigation avant/arrière entre sous-menus. **Menu principal (sur un mot)** : couper/copier/coller, correction orthographe/grammaire (sous-menu avec suggestions, ignorer, ajouter au dictionnaire), outils d'écriture (sous-menu), rechercher sur Wikipédia/internet (liens externes). **Menu principal (sélection multi-mots)** : couper/copier/coller, outils d'écriture (sous-menu), rechercher sur internet. **Sous-menu outils d'écriture** : transformation de casse (MAJUSCULES/minuscules), et pour un mot : définition, conjugaison, étymologie, champ lexical (liens CNRTL), synonymes/antonymes (résultats inline scrappés de CNRTL avec copier/remplacer, lien « Voir tout »). **Menu natif** : accessible via `⌘+clic droit` (Mac) ou `Ctrl+clic droit` (PC), indiqué par un texte discret en bas du menu. **Implémentation** : ProseMirror Plugin avec `handleDOMEvents.contextmenu`, DOM vanilla (pas de React) pour les menus, positionnement dynamique avec overflow protection, guard `menuOpenedAt` (300ms) pour éviter la fermeture au relâchement du clic droit. **Fichiers** : `src/lib/nspell-instance.ts` (singleton, tokenizer français, gestion mots custom), `src/lib/spellcheck-extension.ts` (extension TipTap hybride + menu contextuel), `src/types/nspell.d.ts` (déclarations de types), `public/dictionaries/` (fichiers Hunspell FR). **Dépendances** : `nspell`, `dictionary-fr`.
