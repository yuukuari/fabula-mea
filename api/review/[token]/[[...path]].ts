...
/**
 * Extract the sub-path segments after /api/review/:token/
 * Uses req.query.path (Vercel catch-all param) with fallback to req.url parsing.
 */
function getSubPath(req: VercelRequest, token: string): string[] {
  // Vercel populates req.query.path for [[...path]] catch-all routes
  ...
  const base = `/api/review/${token}`;
  if (url.startsWith(base)) {
    return url.slice(base.length).split('/').filter(Boolean);
  }
  // Relative URL (e.g. "/start" or "start")
...
async function handleComments(req: VercelRequest, res: VercelResponse, token: string) {
  ...
  if (req.method === 'POST') {
    const body = (req.body || {}) as Partial<ReviewComment>;
    const commentsJson = await redis.get(`emlb:review:${session.id}:comments`);
    const comments: ReviewComment[] = commentsJson ? JSON.parse(commentsJson) : [];
...
async function handleCommentById(req: VercelRequest, res: VercelResponse, token: string, commentId: string) {
  ...
  if (req.method === 'PATCH') {
    ...
    if (status !== undefined) comments[idx].status = status as ReviewComment['status'];
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // token is a named Vercel route parameter — always populated
  const token = req.query.token as string;
  if (!token) return res.status(400).json({ error: 'Token requis' });

  // sub-path segments after /api/review/:token/
  const subPath = getSubPath(req, token);
  ...
  if (action === 'start')    return handleStart(req, res, token);
  if (action === 'complete') return handleComplete(req, res, token);
  if (action === 'send')     return handleSend(req, res, token);

  if (action === 'comments' && subPath.length === 1) return handleComments(req, res, token);
  if (action === 'comments' && subPath.length === 2) return handleCommentById(req, res, token, subPath[1]);
